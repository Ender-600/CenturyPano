...
应用主入口现在提供四个模式 Tab，默认相机、共享目标年份 1926。第三个 Tab 单独生成 Street View 历史全景，第四个 Tab 再生成 Marble 世界；Tab 切换和年份滚动都不会提交生成。本文其余独立查看器操作仍适用于 `/world/`，合并后的模式和生命周期规则见 [四模式说明](FOUR_MODES.md)。
