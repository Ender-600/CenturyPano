 ...
 * iOS requestPermission synchronously, before returning its promise. Construction
 * never starts sensors, and permission requests require this explicit gesture API.
 * startAutomatically never requests permission and can resume an earlier grant.
...
export function createOrientationController({
  ...
  compassTimeoutMs = 12000,
} = {}) {
  let status = { enabled: false, phase: 'idle', mode: null, reference: null, heading: null, physicalHeading: null, accuracy: null, northInitialized: false, relativeOnly: false, compassAligning: false, message: 'Drag to look around · Allow motion to follow your phone' };
  let epoch = 0, pending = null, timer = null, latest = null, target = null, disposed = false;
  let permissionGranted = false, permissionVersion = 0, compassRequestedAt = null, compassTimer = null;
  ...
  const armTimer = () => {
    ...
    timer = (win?.setTimeout?.bind(win) || setTimeout)(() => {
      timer = null;
      if (status.enabled) publish({ phase: 'stale', heading: null, physicalHeading: null, message: 'No recent orientation updates. Turn your phone to reconnect, or drag to look around.' });
    }, staleMs);
  };
  ...
  const clearCompassRequest = () => {
    if (compassTimer !== null) (win?.clearTimeout?.bind(win) || clearTimeout)(compassTimer);
    compassTimer = null;
    compassRequestedAt = null;
  };
  const compassPrompt = (sample) => sample?.compassNeedsLevel
    ? 'Hold the phone flat, screen up, to align with a fresh compass reading; then raise it to look around'
    : sample?.compassUnreliable ? 'Waiting for a reliable compass reading · Keep the phone away from magnets'
      : 'Reading the compass to align with your real direction…';
  const applySample = (sample, northSeed = null) => {
    ...
    if (northSeed && (!northOffset || compassRequestedAt !== null)) {
      const alignment = attitudeYawAlignment(sample.quaternion, northSeed.quaternion);
      if (alignment) {
        const explicitAlignment = compassRequestedAt !== null;
        northOffset = alignment;
        northAccuracy = northSeed.accuracy;
        // The first actual north fix must also correct an earlier relative view.
        // Subsequent compass fluctuations never enter look control unless the
        // user explicitly requests a new alignment.
        if (!status.relativeOnly || explicitAlignment) {
    ...
    if (!hasAnchor) {
      if (status.relativeOnly || !northOffset) {
        ...
        if (!alignment) {
          publish({ phase: 'waiting', message: 'Hold your phone upright and point it forward to start following its motion.' });
          return;
        }
    ...
    publish({ phase: 'tracking', mode: calibrated ? 'calibrated' : viewUsesNorth ? 'absolute' : 'relative',
      ...
      compassAligning: compassRequestedAt !== null,
      message: compassRequestedAt !== null ? compassPrompt(sample)
        : calibrated ? 'Manually aligned · Turn your phone to look around'
        : northOffset ? `North initialized${northAccuracy === null ? '' : ` (approx. ±${Math.round(northAccuracy)}°)`} · Turn or tilt your phone to look around${viewUsesNorth ? '' : ' · Align with compass to match the view'}`
          : sample.compassNeedsLevel ? 'Relative follow · Briefly hold the phone flat, screen up, to initialize the compass'
            : sample.compassUnreliable ? 'Compass is unstable · Following phone motion while waiting for a reliable north reading'
              : 'Following phone motion · Waiting for a reliable compass reading' });
  };
  function handleOrientation(event) {
    ...
    if (!sample) return;
    // Never fulfill a compass request with a queued event captured before the
    // tap, or by replaying the cached event on a screen-rotation notification.
    const northSeed = sample.northQuaternion && (compassRequestedAt === null || stamp === null || stamp >= compassRequestedAt)
      ? { quaternion: sample.northQuaternion, accuracy: sample.accuracy } : null;
    ...
    if (sample.source === 'absolute' && current - relativeAt < 1000) {
      if ((!northOffset || compassRequestedAt !== null) && latest && northSeed && current - receivedAt < 100) {
        applySample(latest, northSeed);
      }
      return;
    }
  ...
  function stop(phase = 'idle') {
    ...
    publish({ enabled: false, phase, mode: null, reference: null, heading: null, physicalHeading: null, accuracy: null, northInitialized: false, compassAligning: false,
      message: phase === 'paused' ? 'Motion will reconnect when you return.' : 'Drag to look around · Allow motion to follow your phone' });
  }
  function start({ relativeOnly = false } = {}, requestPermission = true) {
    ...
    if (doc?.visibilityState === 'hidden') return Promise.resolve(false);
    const needsPermission = typeof win.DeviceOrientationEvent.requestPermission === 'function';
    if (!requestPermission && needsPermission && !permissionGranted) return Promise.resolve(false);
    const shouldRequestPermission = needsPermission && requestPermission && !permissionGranted;
    const requestedVersion = shouldRequestPermission ? ++permissionVersion : permissionVersion;
    ...
    try {
      ...
      permission = shouldRequestPermission
    ...
    pending = Promise.resolve(permission).then((result) => {
      // A permission prompt may finish after a view switch or backgrounding
      // cancelled this sensor session. Remember the real grant for a later
      // automatic start, but never revive the cancelled listener/session here.
      // An older prompt also cannot override a newer permission decision.
      if (requestedVersion === permissionVersion && result === 'granted') permissionGranted = true;
      if (epoch !== requestEpoch || disposed) return false;
      pending = null;
      if (result !== 'granted') {
        detach();
        publish({ enabled: false, phase: 'denied', compassAligning: false, message: 'Orientation permission is off. You can still drag to look around.' });
        return false;
      }
      ...
      pending = null;
      detach();
      publish({ enabled: false, phase: 'denied', compassAligning: false, message: 'Orientation permission is unavailable. You can still drag to look around.' });
      return false;
    });
    return pending;
  }
  ...
  function alignToCompass() {
    if (!status.enabled || disposed || doc?.visibilityState === 'hidden') return false;
    clearCompassRequest();
    compassRequestedAt = now();
    publish({ compassAligning: true, message: compassPrompt(latest) });
    compassTimer = (win?.setTimeout?.bind(win) || setTimeout)(() => {
      compassTimer = null;
      compassRequestedAt = null;
      if (status.enabled) publish({ compassAligning: false,
        message: 'Compass alignment could not finish. Hold the phone flat, screen up, and try Align with compass again.' });
    }, compassTimeoutMs);
    return true;
  }
  return { startFromGesture: (options) => start(options),
    startAutomatically: (options) => start(options, false),
    stop, calibrate, alignToCompass, getQuaternion,
    getStatus: () => ({ ...status }),
