"""GPT Image editing via the official Images API.
...
class OpenAIImageEditor:
    ...
    async def edit(
        ...
        del strength, seed  # Unsupported by the Images edits endpoint.
        if not self.api_key:
        ...
        instructions = [
            prompt,
            "Edit image 1 only. Preserve image 1's exact viewpoint, composition, projection, "
            "horizon and full frame. Return one edited image without cropping, extending the scene, "
            "adding a border or creating a collage.",
        ]
        if structure_lock:
            instructions.append(
                "Keep every existing structure at the same footprint, size, position and outline. "
                "Do not add, remove, resize or replace any building; apply the historical brief through "
                "surface materials, facade details, lighting and period objects."
            )
        else:
            instructions.append(
                "Historical reconstruction may reshape, remove or replace buildings and roads when "
                "the historical brief requires it; keep major masses in broadly similar positions "
                "so neighbouring panorama tiles can stitch."
            )
        files = [("image[]", ("source.jpg", image, "image/jpeg"))]
        if reference is not None:
            files.append(("image[]", ("anchor-reference.jpg", reference, "image/jpeg")))
            instructions.append(
                "Image 2 is a style reference only, not historical evidence. Match its lighting, palette and sky while "
        ...
        fields = {
            ...
            "prompt": "\n\n".join(instructions),
            "n": "1",
            "size": _output_size(source_size) if self.model.startswith("gpt-image-2.5") else "1024x1024",
