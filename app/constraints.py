...
async def build_constraints(place: dict, decade: str | int, scene: dict, *, provider: str | None = None,
    ...
    text_configured = (
        (getattr(settings, "k2_api_key", "") and getattr(settings, "k2_base_url", "") and getattr(settings, "k2_model", ""))
    ...
    if location.get("precision") != "coordinates":
        ...
        history["site_history"] = (
            "Without a precise capture location, the development state of this particular "
            "parcel in the reference year cannot be confirmed."
        )
        history["uncertainties"].append(
            "City-wide context is not evidence for the history of a particular site; "
            "provide the capture location and check archival sources."
        )
        if not history.get("literary_summary"):
            history["literary_summary"] = conservative["literary_summary"]
    history["uncertainties"].append(
        "July 1 of the selected year is used as the reference date by default; the scene "
        "may have looked different before or after a midyear change."
    )
    ...
    if not fallback:
        context["uncertainties"].append(
            "Model historical knowledge is unverified; building turnover and land use need historical maps or photographs."
        )
