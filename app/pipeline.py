...
async def run_job(job_id: str, *, concurrency: int | None = None, use_cache: bool = True) -> dict:
    ...
    try:
        ...
        def finish_images(failed: frozenset[int]):
            ...
            # 1) Force identical geometry in every overlap (railings / wires).
            # 2) Remove residual tonal drift across the band.
            # 3) Carve or feather whatever structural mismatch remains.
