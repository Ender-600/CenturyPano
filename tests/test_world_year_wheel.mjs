...
function fixture({ min = 1800, max = 2026, value = '1925', initialRowHeight = 36, initiallyHidden = false } = {}) {
  const timers = new Map(), observers = [];
  let nextTimer = 0, rowHeight = initialRowHeight, hidden = initiallyHidden;
  ...
  class Element {
    ...
    get offsetHeight() { return hidden ? 0 : rowHeight; }
    get clientHeight() { return hidden ? 0 : this.layoutHeight; }
    set clientHeight(height) { this.layoutHeight = height; }
    get scrollTop() { return hidden ? 0 : this.offset || 0; }
    set scrollTop(offset) { this.offset = hidden ? 0 : offset; }
  ...
  return { wheel, element, input, changes, timers, observers,
    ...
    hide() { hidden = true; element.offset = 0; for (const observer of observers) observer.callback(); },
    show() { hidden = false; for (const observer of observers) observer.callback(); },
    scroll(year) { element.emit('wheel', { deltaY: 1 }); element.scrollTop = (year - Number(input.min)) * rowHeight; element.emit('scroll'); },
  };
}
...
test('initializing before CSS layout retains the chosen year when real row dimensions arrive', () => {
  ...
  assert.equal(view.input.value, '1925');
  assert.equal(view.element.scrollTop, 0);
  view.resize(30);
...
test('hidden initialization and server range updates never publish the collapsed zero scroll position', () => {
  const view = fixture({ value: '1926', initiallyHidden: true });
  assert.equal(view.element.clientHeight, 0);
  assert.equal(view.element.scrollTop, 0);
  view.element.emit('scroll'); view.element.emit('scrollend'); view.settle();
  view.wheel.setRange(1800, 2025);
  view.element.emit('scroll'); view.element.emit('scrollend'); view.settle();
  assert.equal(view.wheel.commit(), 1926);
  assert.equal(view.input.value, '1926');
  assert.equal(view.element.attributes['aria-valuenow'], '1926');
  assert.deepEqual(view.changes, []);
  view.show();
  assert.equal(view.element.scrollTop, 126 * 36);
  view.element.emit('scroll'); view.element.emit('scrollend'); view.settle();
  assert.equal(view.input.value, '1926'); assert.deepEqual(view.changes, []);
});

test('hiding and showing a wheel cancels unfinished scrolling and restores its committed year', () => {
  const view = fixture({ value: '1926' });
  view.element.emit('touchstart'); view.scroll(1950);
  assert.equal(view.element.attributes['aria-valuenow'], '1950');
  view.hide(); view.element.emit('scroll'); view.element.emit('touchend'); view.settle();
  assert.equal(view.input.value, '1926');
  assert.equal(view.element.attributes['aria-valuenow'], '1926');
  view.show(); view.element.emit('scroll'); view.element.emit('scrollend'); view.settle();
  assert.equal(view.element.scrollTop, 126 * 36);
  assert.deepEqual(view.changes, []);
  view.scroll(1940); view.settle();
  assert.equal(view.input.value, '1940'); assert.deepEqual(view.changes, [1940]);
});

test('silent programmatic updates and relayout scroll events cannot become user selections', () => {
  const view = fixture({ value: '1926' });
  view.wheel.setValue(1970);
  // A queued browser scroll from layout may report an offset from before setValue.
  view.element.scrollTop = 0; view.element.emit('scroll'); view.element.emit('scrollend'); view.settle();
  assert.equal(view.input.value, '1970'); assert.equal(view.element.scrollTop, 170 * 36);
  view.hide(); view.wheel.setValue(1950); view.wheel.setRange(1810, 2026);
  view.element.emit('scroll'); view.settle(); view.show();
  view.element.emit('scroll'); view.element.emit('scrollend'); view.settle();
  assert.equal(view.input.value, '1950'); assert.equal(view.element.scrollTop, 140 * 36);
  assert.deepEqual(view.changes, []);
  view.element.emit('pointerdown');
  view.element.scrollTop = 150 * 36; view.element.emit('scroll'); view.settle();
  assert.equal(view.input.value, '1960'); assert.deepEqual(view.changes, [1960]);
});
