...
def test_gemini_sends_two_images_and_normalizes_output():
    ...
    for licence in ("but remove or replace buildings", "recompose", "move the viewpoint"):
        assert licence not in instruction.lower()
    assert "major masses" in instruction.lower()
    assert "historically justified" in instruction.lower()
