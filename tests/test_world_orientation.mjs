...
test('upright startup follows motion and the first reliable north reading aligns the view', async () => {
...
test('a concurrent first absolute sample aligns north and later compass changes do not replace relative look', async () => {
...
test('Align heading obtains fresh compass north for each cardinal direction, then follows gyro deltas', async () => {
  for (const heading of [0, 90, 180, 270]) {
    const f = fixture({ viewerHeading: 145 }); await f.controller.startFromGesture();
    f.send({ alpha: 20 });
    f.send({ alpha: 300 }, 'deviceorientationabsolute');
    near(headingFromQuaternion(f.controller.getQuaternion()), 60);
    f.camera(215); // A dragged/previous view is never a compass measurement.
    f.advance(1);
    assert.equal(f.controller.alignToCompass(), true);
    assert.equal(f.controller.getStatus().compassAligning, true);
    near(headingFromQuaternion(f.controller.getQuaternion()), 60);
    f.send({ alpha: 15 });
    near(headingFromQuaternion(f.controller.getQuaternion()), 65);
    assert.equal(f.controller.getStatus().compassAligning, true);
    f.send({ alpha: (360 - heading) % 360 }, 'deviceorientationabsolute');
    near(headingFromQuaternion(f.controller.getQuaternion()), heading);
    near(f.controller.getStatus().physicalHeading, heading);
    assert.equal(f.controller.getStatus().compassAligning, false);
    f.send({ alpha: 5 });
    near(headingFromQuaternion(f.controller.getQuaternion()), (heading + 10) % 360);
    f.send({ alpha: 111 }, 'deviceorientationabsolute');
    near(headingFromQuaternion(f.controller.getQuaternion()), (heading + 10) % 360);
    f.controller.dispose();
  }
});

test('Align heading ignores cached screen events and queued old compass readings', async () => {
  const f = fixture(); await f.controller.startFromGesture();
  f.send({ alpha: 270, absolute: true });
  f.advance(100);
  f.controller.alignToCompass();
  f.screen.angle = 90; f.screen.emit('change');
  assert.equal(f.controller.getStatus().compassAligning, true);
  f.send({ alpha: 180, absolute: true, timeStamp: 10001 });
  assert.equal(f.controller.getStatus().compassAligning, true);
  // Relative attitude may still turn during alignment, but cannot fulfill it.
  f.send({ alpha: 20 });
  const before = f.controller.getQuaternion();
  f.send({ alpha: 0, timeStamp: 10100 }, 'deviceorientationabsolute');
  assert.equal(f.controller.getStatus().compassAligning, false);
  near(f.controller.getStatus().physicalHeading, 0);
  assert.ok(before.angleTo(f.controller.getQuaternion()) > 0.1);
  f.controller.dispose();
});

test('Safari alignment rejects upright or unreliable compass and waits for a new level reading', async () => {
  const f = fixture(); await f.controller.startFromGesture();
  f.send({ alpha: 10, beta: 0, webkitCompassHeading: 90, webkitCompassAccuracy: 5 });
  f.send({ alpha: 10, beta: 90 });
  f.controller.alignToCompass();
  for (const data of [
    { beta: 90, webkitCompassAccuracy: 5 },
    { beta: 0, webkitCompassAccuracy: -1 },
    { beta: 0, webkitCompassAccuracy: 70 },
  ]) {
    f.send({ alpha: 10, webkitCompassHeading: 270, ...data });
    assert.equal(f.controller.getStatus().compassAligning, true);
  }
  f.send({ alpha: 10, beta: 0, webkitCompassHeading: 270, webkitCompassAccuracy: 5 });
  assert.equal(f.controller.getStatus().compassAligning, false);
  f.send({ alpha: 10, beta: 90, webkitCompassHeading: 90, webkitCompassAccuracy: 5 });
  near(headingFromQuaternion(f.controller.getQuaternion()), 270);
  near(f.controller.getStatus().physicalHeading, 270);
  f.controller.dispose();
});

test('unavailable compass alignment times out without inventing a north reading or losing gyro look', async () => {
  const f = fixture({ viewerHeading: 80 }); await f.controller.startFromGesture();
  f.send({ alpha: 10 }); f.controller.alignToCompass();
  f.advance(12001);
  assert.equal(f.controller.getStatus().compassAligning, false);
  assert.equal(f.controller.getStatus().physicalHeading, null);
  assert.match(f.controller.getStatus().message, /could not finish/);
  near(headingFromQuaternion(f.controller.getQuaternion()), 80);
  f.send({ alpha: 0 }); near(headingFromQuaternion(f.controller.getQuaternion()), 90);
  f.controller.dispose();
  assert.equal(f.timers.size, 0);
});

test('automatic startup does not request permission and resumes a previous iOS grant without another prompt', async () => {
  const f = fixture({ permission: () => 'granted' });
  assert.equal(await f.controller.startAutomatically(), false);
  assert.equal(f.calls(), 0); assert.equal(f.win.total(), 0);
  assert.equal(await f.controller.startFromGesture(), true);
  ...
  assert.equal(await f.controller.startAutomatically(), true);
  assert.equal(f.calls(), 1);
  assert.equal(f.controller.getQuaternion(), null);
  f.send({ alpha: 180, absolute: true });
  near(headingFromQuaternion(f.controller.getQuaternion()), 180);
  f.controller.dispose();
});

test('automatic startup starts no-prompt browsers, but denied iOS permission never auto-prompts again', async () => {
  const automatic = fixture();
  assert.equal(await automatic.controller.startAutomatically(), true);
  automatic.send({ alpha: 270, absolute: true });
  near(headingFromQuaternion(automatic.controller.getQuaternion()), 90);
  automatic.controller.dispose();
  const denied = fixture({ permission: () => 'denied' });
  assert.equal(await denied.controller.startFromGesture(), false);
  assert.equal(await denied.controller.startAutomatically(), false);
  assert.equal(denied.calls(), 1); assert.equal(denied.win.total(), 0);
...
test('a grant after session cancellation is reusable without reviving the cancelled sensor session', async () => {
  for (const cancel of ['hidden', 'view-change']) {
    let finish;
    const f = fixture({ permission: () => new Promise((resolve) => { finish = resolve; }) });
    const pending = f.controller.startFromGesture();
    if (cancel === 'hidden') { f.doc.visibilityState = 'hidden'; f.doc.emit('visibilitychange'); }
    else f.controller.stop();
    finish('granted'); assert.equal(await pending, false);
    assert.equal(f.win.total(), 0); assert.equal(f.controller.getStatus().enabled, false);
    f.doc.visibilityState = 'visible';
    assert.equal(await f.controller.startAutomatically(), true);
    assert.equal(f.calls(), 1);
    f.send({ alpha: 270, absolute: true });
...
test('a late old grant cannot override a newer denied permission decision', async () => {
  const finishes = [];
  const f = fixture({ permission: () => new Promise((resolve) => { finishes.push(resolve); }) });
  const older = f.controller.startFromGesture();
  f.controller.stop();
  const newer = f.controller.startFromGesture();
  finishes[1]('denied'); assert.equal(await newer, false);
  finishes[0]('granted'); assert.equal(await older, false);
  assert.equal(await f.controller.startAutomatically(), false);
  assert.equal(f.calls(), 2); assert.equal(f.win.total(), 0);
  f.controller.dispose();
});
