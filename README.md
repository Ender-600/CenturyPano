...
**单街区历史世界工作台**现在默认使用手机实时 GPS：获取附近 Google Street View 的 RGB 360° 全景照片 → GPT Image 历史改图 → Marble Draft → Spark 三维查看。这里的输入是街景照片，详见 [Street View 接入说明](docs/STREETVIEW_INPUT.md)。旧 OSM 粗模型／深度链路仅保留在显式测试模式与旧任务回放中，其实验记录见 [几何路线说明](docs/MARBLE_IMPLEMENTATION.md)。
