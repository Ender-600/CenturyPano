...
街景与历史全景支持透视环视，切换时保留查看方向；手机可开启转动跟随，并对准眼前地标手动校准朝向。GPS 行动方向单独显示。这仍不是 AR 位置追踪：拍摄点偏移、指南针误差、AI 改图和生成世界坐标差异会影响对齐，实体手机效果待验收。操作与边界见 [手机时间视窗](docs/WORLD_VIEWER.md)。旧 M0 单图实验的费用和时长不代表此链路；当前记录见 [实现与验收说明](docs/MARBLE_IMPLEMENTATION.md)。
