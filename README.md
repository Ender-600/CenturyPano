...
打开 <http://localhost:8000>。点击「换张全景」选择照片，预览、选择年份，再点击「走进年份」生成。点击画面中的年份或右上角菜单，可用滑杆或直接输入选择精确年份；菜单中的「先来一场时光旅行」可打开工程示例。
...
拍照、城市与 360° 设置、历史背景、生成数据、音频和下载位于右上角菜单。上传时读取照片 GPS；「使用当前位置」主动请求设备定位。手填城市优先于照片位置与设备定位。离开生成页面后，同一浏览器的时光档案可以重新打开正在进行的任务。纯净模式隐藏控件，点击右上角眼睛或按 Escape 恢复。
...
年份滑杆按 1 年移动，也可直接输入年份；默认 1920。每年以 **7 月 1 日** 为明确参考时点，避免把年内事件前后混成一幅图。1945 和 1950 会分别根据位置推理，不能只套用相同的年代风格，也不能把一场战争的状态套用到所有城市。
...
node --test tests/test_web.mjs tests/test_motion*.cjs
```
