...
页面沿用 `main` 的简洁风格，以「当前街景／历史全景／生成世界」全屏视图为主体，位置和高级选项收进设置。首次打开请求手机 GPS，此后持续更新位置；配置、认证和定位就绪后自动准备附近街景。查看当前街景时，移动至少约 30 米才自动换景，请求至少间隔 15 秒；历史世界、生成任务和真实行走不会被自动街景更新打断。年份使用上下滑动的滚轮，历史改图和 Marble 世界生成只在点击「走进 … 年」后提交。定位拒绝或超时不会改用 CMU、旧任务或默认坐标；手工点位需要切换测试模式，回放不会覆盖当前定位输入。
...
街景与历史全景支持透视环视，切换时保留查看方向；手机可开启转动跟随，并对准眼前地标手动校准朝向。生成世界加载后，可在设置中开启真实行走：自动模式在 [iOS／Android 客户端](mobile/README.md) 优先使用 ARKit／ARCore，在普通浏览器使用 GPS 近似跟随室外步行。GPS 按经纬度变化映射水平位移，视角仍独立跟随手机转动；需要通过当前指南针或手动已知方位固定朝向映射，并使用米制元数据或两点／手动尺度校准。定位筛选、抖动过滤与插值减少跳动，数据过期或后台暂停会冻结位置并要求重设起点。GPS 更新频率和精度由浏览器与设备决定，不保证逐步即时跟随。[W3C Geolocation](https://www.w3.org/TR/geolocation/)

原生构建与逻辑测试通过；GPS／AR 实体手机追踪和原生安装签名仍待验收。拍摄点、AI 几何与地理朝向误差不会被自动消除。操作与边界见 [手机时间视窗](docs/WORLD_VIEWER.md) 和 [真实行走说明](docs/REAL_WALKING.md)。旧 M0 单图实验的费用和时长不代表此链路；当前记录见 [实现与验收说明](docs/MARBLE_IMPLEMENTATION.md)。

2026-09-12 的一次 CMU 1925 **旧几何路线**真实运行完成了深度转 RGB、Draft 和资产保存，两个阶段各提交 1 次，共扣 230 credits。端到端约 **2 分 29 秒**，包含中途兼容修复和重启等待；两段生成合计约 37 秒不能当成全链路耗时。粗模型的四项建筑移除检查通过，但生成全景仍有现代多车道道路、广告及街区外观，**历史视觉验收未通过**。

旧路线的实际 SPZ 已在 Chrome WebGL2 / ANGLE Metal 的 **Apple M1 Pro GPU** 上显示，并检查了虚拟移动、1280×900 桌面及 390×844 移动视口；画面仍模糊，这不是实体手机 GPU 或 AR 验收。失败记录和页面警告保留。随后完成的 `depth-history-v2` 提示词修复已进入计划缓存与生成版本，但**尚未追加付费验证**，旧任务不会自动重跑。

本次 GPS／Street View 改动的离线验证：完整 Python 套件 494 项、前端 39 项及 Ruff 通过；随后新增的来源路径保护和地点提示词回归测试也通过。Google、OpenAI 与 Marble 的新照片链路使用模拟响应测试，尚未完成真实 Google 全景的端到端验收。

Chrome 浏览器以模拟 GPS 验证了默认定位、准备前重新定位、定位拒绝时停止、显式测试点位，以及旧世界回放不覆盖当前位置；390×844 与 1280×900 页面无横向溢出。这些检查没有调用付费生成，也不替代实体手机 GPS／AR 验收。
