...
页面沿用 `main` 的简洁风格，以「当前街景／历史全景／生成世界」全屏视图为主体，位置和高级选项收进设置。首次打开请求手机 GPS；配置、认证和定位就绪后，新页面自动准备一次附近街景。历史改图和 Marble 世界生成只在点击「走进 … 年」后提交。定位拒绝或超时不会改用 CMU、旧任务或默认坐标；手工点位需要切换测试模式，回放不会覆盖当前定位输入。
